...
class ColorRecorder(BaseRecorder):
    def __init__(self):
        super().__init__()
    ...
    def record(self, tracked_objects: Dict[str, TrackedObject]) -> None:
        ...
        for marker_name, tracked_object in tracked_objects.items():
            frame_data[marker_name] = {
                ...
                "area": tracked_object.extra.get("area", 0),
                "color_match_percentage": tracked_object.extra.get("color_match_percentage", 0),
    ...
    def process_tracked_objects(self, **kwargs) -> np.ndarray:
        ...
        Returns:
            Array of shape (num_frames, num_markers, 4) where last dimension is:
            [pixel_x, pixel_y, area, color_match_percentage]
        ...
        self.recorded_objects_array = np.full(
            (num_frames, num_markers, 4), np.nan, dtype=np.float32
        )
        ...
        for frame_idx, frame_data in enumerate(self.recorded_objects):
            for marker_idx, marker_name in enumerate(self.marker_names):
                if marker_name in frame_data:
                    ...
                    self.recorded_objects_array[frame_idx, marker_idx, 2] = point_data["area"]
                    self.recorded_objects_array[frame_idx, marker_idx, 3] = point_data["color_match_percentage"]
    ...
    def get_metadata_array(self) -> np.ndarray:
        """
        Get just the metadata (area, color_match_percentage).
